...
// Function to validate the username (max 8 characters, alphanumeric only)
...
// Function to validate the subject (max 80 characters)
...
// Sends an email to a specified receiver (sender already known)
...
// List all emails for the logged in user
...
// Read a specific email for the logged in user
...
// Delete a specific email for the logged in user
...
int main(int argc, char **argv)
{
    ...
    // Ensure proper usage with server IP and port
    if (argc < 3)
    {
    ...
    do
    {
        ...
        if (std::cin.getline(buffer, BUF))
        {
            ...
            else if (command == "SEND")
            {
                ...
                // Collect receiver, subject, and message details
                while (true)
                {
                    std::cout << "Receiver: ";
